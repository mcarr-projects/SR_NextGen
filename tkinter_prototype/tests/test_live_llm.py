...
import db_lib
import gemini_calls, oai_calls


LIVE_GEMINI_TESTS_ENABLED = os.getenv("RUN_LIVE_GEMINI_TESTS") == "1"
LIVE_OAI_TESTS_ENABLED = os.getenv("RUN_LIVE_OAI_TESTS") == "1"

GEMINI_TEST_PROMPT = "Reply with exactly: Gemini API connection successful."
OAI_TEST_PROMPT = "Reply with exactly: OpenAI API connection successful."


@unittest.skipUnless(
    LIVE_GEMINI_TESTS_ENABLED,
    "set RUN_LIVE_GEMINI_TESTS=1 to make a real Gemini API call"
)
class TestGeminiLive(unittest.TestCase):
    def test_live_gemini_call_is_recorded_in_real_database(self):
        self.assertTrue(
            os.getenv("GOOGLE_API_KEY"),
            "GOOGLE_API_KEY is missing from the environment or .env"
        )
        ...
        self.assertEqual(recorded_request["input"], GEMINI_TEST_PROMPT)

@unittest.skipUnless(
    LIVE_OAI_TESTS_ENABLED,
    "set RUN_LIVE_OAI_TESTS=1 to make a real OpenAI API call"
)
class TestOAILive(unittest.TestCase):
    def test_live_oai_call_is_recorded_in_real_database(self):
        self.assertTrue(
        ...
        result = oai_calls.call_oai(
            prompt=OAI_TEST_PROMPT,
            purpose="live_test",
            session_id=session_id
        )
        ...
        self.assertIsNotNone(row, "OpenAI call was not recorded in llm_calls")
        self.assertEqual(row["id"], result["llm_call_id"])
        self.assertEqual(row["user_id"], db_lib.DEFAULT_USER_ID)
        self.assertEqual(row["session_id"], session_id)
        self.assertEqual(row["purpose"], "live_test")
        self.assertEqual(row["provider"], oai_calls.PROVIDER)
        self.assertEqual(row["model"], oai_calls.OAI_MODEL)
        self.assertEqual(row["status"], "completed")
        self.assertIsNone(row["error_message"])
        self.assertIsNotNone(row["provider_request_id"])
        self.assertEqual(row["response_text"], result["response_text"])
        ...
        recorded_request = json.loads(row["request_json"])
        self.assertFalse(recorded_request["store"])
        self.assertEqual(recorded_request["input"], OAI_TEST_PROMPT)
        self.assertEqual(
            recorded_request["reasoning"]["effort"],
            oai_calls.REASONING_EFFORT
        )
